let
  ...
  packages = {
    # TODO: inherit mergeLinuxConfig generateLinuxConfig;
    ...
    nvidia-patch = { stdenvNoCC, fetchFromGitHub, fetchpatch, writeShellScriptBin'build, lib, xorg'lndir'build, nvidia_x11 ? linuxPackages.nvidia_x11, linuxPackages ? { } }: let
      nvpatch = writeShellScriptBin'build "nvpatch" ''
        set -eu
        patchScript=$1
        objdir=$2
    ...
    in stdenvNoCC.mkDerivation {
      ...
      src = fetchFromGitHub {
        owner = "keylase";
        repo = "nvidia-patch";
        rev = "357c27e7b35e9c6fdab0a20ca6657fcdf75610b2";
        sha256 = "sha256-sC9/eDN8aopevP7gNMkteBpnQdXXwb4WQc6SRJbLN8w=";
      };

      nativeBuildInputs = [ nvpatch xorg'lndir'build ];
      patchedLibs = [
