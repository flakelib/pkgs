{ lib, stdenvNoCC, buildPackages, fetchFromGitHub, fetchFromGitLab, fetchFromSourcehut }: with lib; let
  inherit (lib.std) Ty;
in {
  ...
  buildTemplate = { pname ? "base16-template-${slug}", slug, templateData, ... }@args: let
    templateIsPath = isDerivation templateData || Ty.pathlike.check templateData;
